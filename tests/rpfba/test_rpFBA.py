...
class Test_rpFBA(TestCase):
    ...
    def test_fba(self):
        ...
        obj_value, rpsbml = rp_fba(rpsbml, 'RP1_sink', logger=self.logger)
    ...
    def test_fraction(self):
        rpsbml = rpSBML(os_path.join(self.data_path, 'merged.xml'))
        obj_value, rpsbml = rp_fraction(rpsbml, 'biomass', 1.0, 'RP1_sink', 1.0, logger=self.logger)
        self.assertTrue(rpsbml)
    ...
    def test_pfba(self):
        rpsbml = rpSBML(os_path.join(self.data_path, 'merged.xml'))
        obj_value, rpsbml = rp_pfba(rpsbml, 'RP1_sink', logger=self.logger)
        self.assertTrue(rpsbml)
