...
def pathway(
    ...
    for react in [rpsbml.getModel().getReaction(i.getIdRef()) for i in rp_pathway.getListOfMembers()]:
        ...
        logger.debug('Process reaction: '+str(react))
    ...
    if update_rpsbml:
        rpsbml.updateBRSynth(rp_pathway, 'dfG_prime_o'     , np.sum(pathway_standard_dg_prime)        , 'kj_per_mol')
        rpsbml.updateBRSynth(rp_pathway, 'dfG_prime_o_std' , np.std(pathway_standard_dg_prime)        , 'kj_per_mol')
        rpsbml.updateBRSynth(rp_pathway, 'dfG_prime_m'     , np.sum(pathway_physiological_dg_prime)   , 'kj_per_mol')
        rpsbml.updateBRSynth(rp_pathway, 'dfG_prime_m_std' , np.std(pathway_physiological_dg_prime)   , 'kj_per_mol')
        rpsbml.updateBRSynth(rp_pathway, 'dfG_uncert'      , np.mean(pathway_standard_dg_prime_error) , 'kj_per_mol')
        rpsbml.updateBRSynth(rp_pathway, 'dfG_uncert_std'  , np.std(pathway_standard_dg_prime_error)  , 'kj_per_mol')
...
def _reactionStrQuery(libsbml_reaction, rpsbml, cc, update_rpsbml=False, logger=getLogger(__name__)):
    ...
    logger.debug('Reaction: '+str(libsbml_reaction))
    ...
    try:
        ...
        if not reac_str:
            ...
            if update_rpsbml:
                ...
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_o', 0.0, 'kj_per_mol')
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_m', 0.0, 'kj_per_mol')
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_uncert', 0.0, 'kj_per_mol')
                #Are there default values for these?
                #rpsbml.updateBRSynth(libsbml_reaction, 'reversibility_index', 0.0)
                #rpsbml.updateBRSynth(libsbml_reaction, 'balanced', rxn.is_balanced())
            return {}
        ...
        if update_rpsbml:
            rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_o' , results['standard_dg'].value.m            , 'kj_per_mol')
            rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_m' , results['physiological_dg_prime'].value.m , 'kj_per_mol')
            rpsbml.updateBRSynth(libsbml_reaction, 'dfG_uncert'  , standard_dg.error.m                       , 'kj_per_mol')
            if results['ln_reversibility_index']:
                rpsbml.updateBRSynth(libsbml_reaction, 'reversibility_index', results['ln_reversibility_index'])
            rpsbml.updateBRSynth(libsbml_reaction, 'balanced', rxn.is_balanced())
...
#         mnx_default_conc = json.load(open(os_path.join(os_path.dirname(os_path.abspath( __file__ )), 'data', 'mnx_default_conc.json'), 'r'))
#         rpsbml = rpsbml
#         calc_cmp = {}
...
def _makeSpeciesStr(libsbml_species, rpsbml, ret_type='xref', logger=getLogger(__name__)):
    ...
    logger.debug('Species: '+str(libsbml_species))
...
def _makeReactionStr(libsbml_reaction, rpsbml, ret_type='xref', ret_stoichio=True, logger=getLogger(__name__)):
...
def _speciesCmpQuery(libsbml_species, rpsbml, cc, calc_cmp, logger=getLogger(__name__)):
    ...
    elif spe_cmp.id == 4: # this is H+ and can be ignored
        logger.debug(str(spe_cmp) + ' is H+ and can be ignored')
        return 'h', 'h'

    logger.debug('spe_id: '+str(spe_id))
    logger.debug('spe_cmp: '+str(spe_cmp))

    # mu -- Formation energy mean estimate (as before)
    # uncertainty_array -- Array representing the uncertainty (a projection in the subspace of known reactions).
    #                      If you take the inner product of that vector with itself, you'll get the variance of the estimate.
    #                      An inner product with the array of another compound would yield the co-variance of their estimates.
    # null_space -- Similar to the second, but is a projection on the null-space.
    #               If the inner product with another such vector is not zero, it means the estimate is completely unreliable
    #               and should not be used.
    mu, uncertainty_array, null_space = cc.predictor.preprocess.get_compound_prediction(spe_cmp)

    return mu, sqrt(np_inner(uncertainty_array, uncertainty_array))


def _reactionCmpQuery(libsbml_reaction, rpsbml, cc, calc_cmp, update_rpsbml=False, physio_param=1e-3, logger=getLogger(__name__)):
    ...
    for rea in libsbml_reaction.getListOfReactants():
        ...
        if not mu:
            ...
            if update_rpsbml:
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_o', None, 'kj_per_mol')
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_m', None, 'kj_per_mol')
                rpsbml.updateBRSynth(libsbml_reaction, 'dfG_uncert', None, 'kj_per_mol')
            return False
    ...
    if update_rpsbml:
        rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_o', dfG_prime_o, 'kj_per_mol')
        rpsbml.updateBRSynth(libsbml_reaction, 'dfG_prime_m', dfG_prime_m, 'kj_per_mol')
        rpsbml.updateBRSynth(libsbml_reaction, 'dfG_uncert', uncertainty, 'kj_per_mol')
...
def toNetworkSBtab(
    ...
    cc: "ComponentContribution",
    outfile: str,
    mnx_default_conc: Dict=None,
    pathway_id: str='rp_pathway',
    thermo_id: str='dfG_prime_o',
    fba_id: str='fba_obj_fraction',
    stdev_factor: float=1.96,
    logger: Logger=getLogger(__name__)):
    """Convert an SBML pathway to a simple network for input to equilibrator-pathway for MDF

    :param outfile: Output path of the TSV file
    :param pathway_id: The id of the heterologous pathway of interest (Default: rp_pathway)
    ...
    if not rp_pathway:
        logger.error('Cannot retrieve the pathway: '+str(pathway_id))
        return False

    # Load 
    if mnx_default_conc is None:
        mnx_default_conc = json_load(
            open(
                os_path.join(
                    os_path.dirname(os_path.abspath( __file__ )),
                    'data',
                    'mnx_default_conc.json'
                ),
                'r'
            )
        )
    ...
    with open(outfile, 'w') as fo:
        ...
        fo.write("p_h\t"+str(cc.p_h)+"\t\t\n")
        fo.write("ionic_strength\t"+str(cc.ionic_strength)+" mM\t\t\n")
        fo.write("p_mg\t"+str(cc.p_mg)+"\t\t\n")
        ...
        ordered_react.sort(key=lambda x: int(x.getId().replace('rxn_', '')), reverse=True)
        # for react in [rpsbml.getModel().getReaction(i.getIdRef()) for i in rp_pathway.getListOfMembers()]:   
        for react in ordered_react:
            react_str = _makeReactionStr(react, rpsbml, ret_type='id', ret_stoichio=True, logger=logger)
            if react_str:
...
def MDF(rpsbml, cc, pathway_id='rp_pathway', thermo_id='dfG_prime_o', fba_id='fba_obj_fraction', stdev_factor=1.96, write_results=True, logger=getLogger(__name__)):
    ...
    with TemporaryDirectory() as tmpOutputFolder:
        path_sbtab = os_path.join(tmpOutputFolder, 'tmp_sbtab.tsv')
        sbtab_status = toNetworkSBtab(
            rpsbml=rpsbml,
            cc=cc,
            outfile=path_sbtab,
        ...
        try:
            ...
            try:
                ...
                rpsbml.updateBRSynth(rp_pathway, 'MDF', 0.0, 'kj_per_mol')
                return 0.0
            ...
            if write_results:
                rpsbml.updateBRSynth(rp_pathway, 'MDF', float(mdf_sol.mdf.m), 'kj_per_mol')
